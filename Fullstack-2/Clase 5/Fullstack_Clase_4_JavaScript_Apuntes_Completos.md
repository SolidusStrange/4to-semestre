...
# Guía de estudio enfocada en JavaScript

> [!important] Prioridad
> En esta clase, JavaScript es el contenido que conviene estudiar con mayor profundidad. HTML y CSS son necesarios para entender el proyecto, pero la lógica principal está en JavaScript.

## 1. El código de la clase, explicado como proceso

```text
HTML crea el formulario
        ↓
JavaScript encuentra el formulario
        ↓
addEventListener escucha "submit"
        ↓
preventDefault evita el envío tradicional
        ↓
JavaScript obtiene .value de cada campo
        ↓
...
se crea el objeto usuario
        ↓
JSON.stringify convierte objeto → texto JSON
        ↓
localStorage.setItem guarda el texto
        ↓
window.location cambia a tabla.html
        ↓
tabla.html usa getItem
        ↓
JSON.parse convierte texto → objeto
        ↓
getElementById obtiene el tbody
        ↓
innerHTML genera HTML
        ↓
${...} inserta los datos del objeto
```

Esta secuencia es la que deberías poder explicar de principio a fin.

---

# JavaScript: conceptos que aparecen realmente en el proyecto
...
Se usa para declarar una variable que no será reasignada.
...
El objeto está declarado con `const`, pero sus propiedades pueden modificarse.
...
A partir de `document` podemos buscar elementos:
...
Busca el elemento HTML cuyo `id` sea `"nombre"`.
...
Obtiene el valor actual introducido en el control.
...
Registra una función para ejecutarla cuando ocurra un evento.

---

## `submit`

Es el evento de envío de un formulario.

```javascript
formulario.addEventListener("submit", ...)
```

El evento ocurre sobre el `<form>`, no sobre el botón de forma independiente. citeturn819805search0
...
Representa el evento que ocurrió.

Permite, entre otras cosas, ejecutar:
...
Evita el comportamiento por defecto del evento y permite que JavaScript controle el proceso. citeturn819805search0turn819805search8
...
Ejecuta el bloque cuando la condición es verdadera.
...
Termina inmediatamente la función actual.

---

# Strings: dos métodos fundamentales de la clase
...
Permite comparar sin depender de mayúsculas/minúsculas.
...
```text
stringify → convierte para almacenar/transmitir
parse     → convierte para volver a utilizar
```

MDN documenta `JSON.stringify()` y `JSON.parse()` precisamente con esta relación. citeturn819805search3turn819805search7
...
`Storage` trabaja con strings; para guardar objetos y arrays normalmente se convierten a JSON primero. citeturn819805search2turn819805search11
...
Los template literals también permiten strings de varias líneas. citeturn819805search10
...
> [!warning] Seguridad
> `innerHTML` interpreta el contenido como HTML. Para texto proveniente de usuarios no confiables, insertar directamente el contenido mediante `innerHTML` puede provocar XSS. Para texto simple suele ser preferible `textContent`.

---

# Solución corregida del proyecto
...
El código anterior asume que existe información en `localStorage`.
...
if (datosGuardados !== null) {
    ...
    console.log("No existe ningún usuario guardado");

}
```

Esto evita intentar hacer `JSON.parse(null)` y acceder después a propiedades de un valor inexistente.

---

# Ejercicios de práctica

## Ejercicio 1 — Capturar un dato
...
Cuando se haga clic en el botón, muestra el nombre en:
...
Recupera una persona desde `localStorage` y genera:
...
## Ejercicio 6 — Botón limpiar

Crea un formulario y un botón:
...
Recórrelos con un `for` y muestra:
...
## Ejercicio 9 — Guardar Array de objetos
...
## Ejercicio 10 — Tabla de usuarios

Utiliza el Array anterior para generar una fila por usuario.
...
# Desafío final — CRUD inicial con LocalStorage

Construye una pequeña aplicación de usuarios.
...
Cada usuario debe tener un botón:

```text
Eliminar
```

que lo quite del Array y vuelva a guardar la lista.

## Actualizar

Agrega la posibilidad de modificar el nombre o correo de un usuario.

Este ejercicio ya reúne prácticamente todos los conceptos importantes de esta clase:
...
# Checklist para saber si dominaste la clase

- [ ] Puedo explicar qué es el DOM.
- [ ] Sé usar `document.getElementById()`.
- [ ] Sé obtener `.value`.
- [ ] Sé utilizar `addEventListener()`.
- [ ] Entiendo qué hace `submit`.
- [ ] Entiendo `event.preventDefault()`.
- [ ] Sé usar `if`, `===`, `||` y `return`.
- [ ] Sé utilizar `trim()`.
- [ ] Sé utilizar `toLowerCase()`.
- [ ] Puedo crear un objeto.
- [ ] Puedo acceder a `objeto.propiedad`.
- [ ] Sé diferenciar objeto y Array.
- [ ] Entiendo `JSON.stringify()`.
- [ ] Entiendo `JSON.parse()`.
- [ ] Puedo usar `localStorage.setItem()`.
- [ ] Puedo usar `localStorage.getItem()`.
- [ ] Puedo usar `localStorage.removeItem()`.
- [ ] Entiendo template literals.
- [ ] Sé usar `${}`.
- [ ] Puedo modificar HTML con `innerHTML`.
- [ ] Sé navegar con `window.location`.
- [ ] Puedo explicar todo el flujo formulario → objeto → JSON → LocalStorage → objeto → HTML.

---

# Resumen de memorización
